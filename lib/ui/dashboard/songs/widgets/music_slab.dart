...
Future<void> fetchTrackInfo() async {
  try {
    ...
    try {
      track = await spotify.tracks.get(_currentTrackId);
    } on FormatException catch (e) {
      print("FormatException — Possibly rate limited or plain text response: $e");
      return; // Exit gracefully
    } catch (e) {
      print("General error while fetching track: $e");
      return;
    }

    // Check again in case track is not what we expect
    if (track == null || track is! Track) {
      print("Invalid or unexpected track data");
      return;
    }
